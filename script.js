...
async function syncProductToSheets(product) {
    ...
    try {
        ...
        console.log(`✅ تم حفظ المنتج ${product.name}`);
    } catch (error) {
...
// ========== إضافة أو تحديث منتج ==========
function addOrUpdateProduct(barcode, name, cost, price) {
    ...
    const newProduct = { barcode, name, cost: parseFloat(cost), price: parseFloat(price) };
...
function renderProductsTable() {
    ...
    if (!filtered.length) { tbody.innerHTML = '<tr><td colspan="5">لا توجد منتجات</td></tr>'; return; }
...
// ========== تسجيل البيع في Google Sheets ==========
async function syncSaleToSheets(product) {
    const payload = {
        action: 'addSale',
        barcode: String(product.barcode),
        name: product.name,
    ...
    try {
        ...
        console.log(`✅ تم تسجيل بيع ${product.name}`);
    } catch (error) {
        console.error(`❌ فشل تسجيل البيع:`, error);
    }
}
...
function checkout() {
    ...
    currentCart.forEach(product => { syncSaleToSheets(product); });
...
function initBarcodeScanner() {
    ...
    if (manualBtn) manualBtn.addEventListener('click', () => { const barcode = scanInput.value.trim(); if (barcode) processScannedBarcode(barcode); else showScanFeedback('الرجاء إدخال الباركود', 'error'); });
    scanInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') { e.preventDefault(); const barcode = scanInput.value.trim(); if (barcode) processScannedBarcode(barcode); } });
...
function processScannedBarcode(barcode) {
    const scanInput = document.getElementById('barcodeScannerInput');
    const product = products.find(p => p.barcode === barcode);
    if (!product) { showScanFeedback(`❌ المنتج غير موجود!`, 'error'); if (scanInput) { scanInput.style.border = '1px solid var(--danger)'; setTimeout(() => { if (scanInput) scanInput.style.border = ''; }, 500); } scanInput.value = ''; scanInput.focus(); return; }
    addToCart(barcode, 1);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (document.getElementById('saveProductBtn')) {
        document.getElementById('saveProductBtn').addEventListener('click', () => {
            ...
            ['prodBarcode', 'prodName', 'prodCost', 'prodPrice'].forEach(id => document.getElementById(id).value = '');
        });
